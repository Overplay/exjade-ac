var JSMpeg = {
    ...
    CreateVideoElements:  function () {
        var elements = document.querySelectorAll( ".ogaudio" );
        for ( var i = 0; i < elements.length; i++ ) {
            new JSMpeg.VideoElement( elements[ i ] )
        }
    },
    CreateOGAudioElement: function () {
        var elements = document.querySelectorAll( ".ogaudio" );
        for ( var i = 0; i < elements.length; i++ ) {
            new JSMpeg.OGAudioElement( elements[ i ] )
        }
    },
...
JSMpeg.OGAudioElement = function () {
    "use strict";

    var ButtonElement = function ( element ) {

        // Grab the `data-url` attrib
        var url = element.dataset.url;
        if ( !url ) {
            throw"VideoElement has no `data-url` attribute"
        }

        // adds additional styles to any element passed in
        ...
        this.canvas.width = 1;  //960
        this.canvas.height = 1; //540
        addStyles( this.canvas, { display: "inline" } );
    ...
    ButtonElement.prototype.onUnlockAudio = function ( element, ev ) {
    ...
    ButtonElement.prototype.onClick = function ( ev ) {
    ...
    ButtonElement.prototype.styleForState = function () {

        if ( !this.player.isPlaying ) {
            this.container.classList.remove( "btn-danger" );
            this.container.classList.add( "btn-success" );
            this.container.innerHTML = "PLAY";
        } else {
            this.container.classList.add( "btn-danger" );
            this.container.classList.remove( "btn-success" );
...
JSMpeg.Player = function () {
    ...
    Player.prototype.getCurrentTime = function () {
        return this.audio && this.audio.canPlay ? this.audio.currentTime - this.audio.startTime : (this.video ? this.video.currentTime - this.video.startTime : 0 );
    };
...
JSMpeg.Source.WebSocket = function () {
    ...
    var WSSource = function ( url, options ) {
        ...
        this.destination = null;
        this.protocol = options.protocol !== undefined ? options.protocol : null;
...
JSMpeg.Demuxer.TS = function () {
    ...
    TS.prototype.parsePacket = function () {
        ...
        var transportError = this.bits.read( 1 ), payloadStart = this.bits.read( 1 ),
            transportPriority = this.bits.read( 1 ), pid = this.bits.read( 13 ),
            transportScrambling = this.bits.read( 2 ), adaptationField = this.bits.read( 2 ),
...
JSMpeg.Decoder.MP2Audio = function () {
    ...
    MP2.prototype.readSamples = function ( ch, sb, part ) {
        var q = this.allocation[ ch ][ sb ], sf = this.scaleFactor[ ch ][ sb ][ part ],
            sample = this.sample[ ch ][ sb ], val = 0;
        if ( !q ) {
